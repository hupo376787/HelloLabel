...
| 矩形 | R | 单击一个角开始，移动鼠标实时预览，再单击另一角完成 |
| 有向矩形 | O | 点第一点 → 点第一条边终点 → 第三点确定宽度 |
| 圆形 | C | 单击圆心开始，移动鼠标实时预览，再单击圆周位置完成 |
| 点 | D | 单击 |
| 直线 | L | 两次单击 |
...
构建阶段由 `desktop/prepare_runtime.py` 准备基础 Python Runtime，并且只安装 `requirements.txt`。Torch、Ultralytics、SAM/SAM2/SAM3 和模型权重**不会**打进基础桌面安装包。

桌面用户选择 **AI → 安装 AI** 后，HelloLabel 会先停止正在运行的 Python 后端，再使用安装包自带的 CPython，在 Electron 用户数据目录中创建独立 `ai-runtime`。AI 包只安装到这个私有 Runtime；系统 Python 和系统 pip 都不会被调用或修改。下次启动时，HelloLabel 会优先使用有效的 `ai-runtime`；如果 AI Runtime 缺失或损坏，会安全回退到安装包自带的基础 Runtime。

桌面版的可写数据全部放在安装目录之外的 HelloLabel 用户数据目录中：

```text
HelloLabel userData/
├─ ai-runtime/       # 可选的独立 AI Python Runtime
├─ models/           # 模型权重
├─ cache/            # pip / Hugging Face / Torch 缓存
├─ config/           # Ultralytics 与桌面配置
└─ data/
```
...
Workflow 会为各平台准备对应架构的独立 CPython 3.12，只安装 `requirements.txt`，再由 Electron Builder 打包。它**不会运行 `install_ai.*`，不会下载 Torch/SAM/YOLO，也不会把模型权重打进安装包**。构建完成后会自动上传到对应 GitHub Release。

程序图标位于 `desktop/build/`：Windows 使用 `icon.ico`、macOS 使用 `icon.icns`、Linux 使用 `build/icons/` 多尺寸 PNG。

更多桌面构建说明见 `desktop/README.md`。

...
function handleDrawPointerDown(ev){
  ...
  if(m==="rectangle"||m==="circle"){state.drawing={type:m,start:p,current:p,dragStartClient:[ev.clientX,ev.clientY]};els.viewport.setPointerCapture?.(ev.pointerId);renderDrawingOverlay();return true;}
...
function handleDrawPointerUp(ev){const d=state.drawing;if(!d)return false;if((d.type==="rectangle"||d.type==="circle")&&d.start){const p=clampImagePoint(screenToImage(ev.clientX,ev.clientY)),moved=Math.hypot(ev.clientX-d.dragStartClient[0],ev.clientY-d.dragStartClient[1]);d.current=p;const type=d.type,points=[d.start,d.current];state.drawing=null;renderDrawingOverlay();if(moved>=3)commitGeometry(type,points);else setStatus(t("tooSmall",{type:shapeTypeText(type)}));return true;}return false;}
...
async function showAbout(){await showModal({title:t("menuAboutHelloLabel"),body:`<div style="white-space:pre-line">${escapeHtml(t("aboutText"))}</div><div class="muted" style="padding-left:0">Version 0.2.12</div>`,buttons:[{label:t("close"),value:"ok",className:"primary"}]});}
async function showShortcuts(){
  ...
  const rows=zh?[
    ...
    ["R","Rectangle: press and drag to create a rectangle."],
    ["O","Oriented Rectangle: click two points to define an edge, then click again to set the width."],
    ["C","Circle: press at the center and drag to the circumference."],

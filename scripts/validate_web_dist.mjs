...
if (exists("VERSION.txt")) {
  assert(read("VERSION.txt").trim() === "HelloLabel 2.2.0 - browser-only static runtime", "VERSION.txt must identify the v2.2.0 browser-only runtime");
}
...
if (errors.length) {
  console.error("HelloLabel v2.2 web distribution validation failed:\n");
  for (const error of errors) console.error(` - ${error}`);
  process.exit(1);
}

console.log("HelloLabel v2.2 web distribution validation passed.");
